...
*/
    ...
    public ArrayList<Map<String, String>> getAll() {
        ...
        while (!cur.isAfterLast()) {
            ...
            map.put(Words.Word._ID,cur.getString(0));
            map.put(Words.Word.COLUMN_NAME_WORD,cur.getString(1));
            map.put(Words.Word.COLUMN_NAME_MEANING,cur.getString(2));
            map.put(Words.Word.COLUMN_NAME_SAMPLE,cur.getString(3));
            cur.moveToNext();
            List.add(map);
        }
        cur.close();
    ...
    public ArrayList<Map<String, String>> ConvertCursor2List(Cursor cur) {
        ArrayList<Map<String, String>> List = new ArrayList<Map<String, String>>();
        if(cur == null)
            return null;
        cur.moveToFirst();
...
// New value for one column
    ...
    private void SearchDialog() {
        final LinearLayout layout = (LinearLayout) getLayoutInflater().inflate(R.layout.searchitem, null);
        new AlertDialog.Builder(this)
                .setTitle("搜索单词")//标题
                .setView(layout)//设置视图
                //确定按钮及其动作
                .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        String txtSearchWord = ((EditText)layout.findViewById(R.id.txtSearch)).getText().toString();
